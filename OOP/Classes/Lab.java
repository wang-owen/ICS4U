public class Lab {
    ...
    public Lab(int max_capcity) {
        this.max_capacity = max_capacity;
        computers = new Computer[max_capacity];
        num_installed = 0;
    }

    public void installComputer(int serial, double speed, int ram, int year_made, int year_purchased,
            int warranty_expiration) {
        if (num_installed >= max_capacity) {
            System.out.println("Cannot install more computers.");
        } else {
            Computer computer = new Computer(serial, speed, ram, year_made, year_purchased, warranty_expiration);
    ...
    public int avgAge() {
        ...
        for (int i = 0; i < num_installed; i++) {
            age += 2022 - computers[i].getYearMade();
        }
    ...
    public Computer newestComputer() {
        ...
        for (int i = 0; i < num_installed; i++) {
            if (computers[i].getYearMade() > latest) {
                ...
                latest = computers[i].getYearMade();
            }
        }
